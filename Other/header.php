...
<body>
    <nav class="navbar sticky-top navbar bg-dark border-bottom border-body" data-bs-theme="dark">
        <div class="container-fluid">
            ...
            <form class="w-50 d-flex" role="search" method="GET" action="../Other/search.php">
                <input class="form-control me-2" type="search" placeholder="Search" aria-label="Search" name="search">
                <button class="btn btn-outline-light" type="submit">Search</button>
            </form>
            <?php if (!isset($_SESSION['username'])) { ?>
                <a href="../Other/login.php" class="btn btn-outline-light">
                    Log in</a><?php
                            }  ?>
            <?php if (isset($_SESSION['username'])) { ?>
                <p class="user">Welcome <?= $_SESSION['username'] ?> !</p><?php
                                                                        }  ?>
            <?php if (isset($_SESSION['username'])) { ?>
                <a href="../Other/logout.php" class="btn btn-outline-light">
                    Log out</a><?php
                            }  ?>

            <a href="../Other/detail_cart.php" class="btn btn-outline-light">
                Cart
